...
MISSKEY_TOKEN="Rv0WhSOwv2Fa8PRO30I7aOcMfeK4v8n9"
MISSKEY_HOST="misskey.until.tsukuba.one"
POETRY=$(which poetry)

USERID_LIST_FILE=$(mktemp)
echo "USERID_LIST_FILE: $USERID_LIST_FILE" 1>&2
$POETRY run python3 src/fetchuserlists.py --host "$MISSKEY_HOST" --token "$MISSKEY_TOKEN" \
...
cat "$USERID_LIST_FILE"  \
    | xargs -I {} -P 4 -t sh -c 'sleep 1 && '"$POETRY"' run python3 src/fetchusernotes.py -u {} --host '"$MISSKEY_HOST"' >> '"$NOTEID_LIST_FILE"
"$POETRY" run python3 src/filternotes.py "$NOTEID_LIST_FILE" | xargs -I {} -P 4 echo "https://$MISSKEY_HOST/notes/"{}
